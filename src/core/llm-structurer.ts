...
function createPrompt(rawText: string, mode: Mode): string {
  return [
    ...
    "2.1.1) `第 1 章 绪论`、`第1章` 等带空格写法仍按一级标题处理。",
    ...
    "7) 若识别到公式/方程，须单独成行输出为 paragraph，并在行末补连续编号 `(1)` `(2)`...；重复公式不得新编号。",
    "8) 长公式优先在 `=` 处换行；若无法在 `=` 处换行，可在 `+` `-` `·` 等符号处换行，换行符号置于下一行行首。",
    "9) 图题与表题按 caption 输出为 paragraph：`表1 标题`（普通空格）与 `图1　标题`（一个汉字空格），标题末尾无标点。",
    "",
    "原始文本如下：",
    rawText,
  ].join("\n");
